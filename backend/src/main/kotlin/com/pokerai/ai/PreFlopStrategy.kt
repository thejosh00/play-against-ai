...
import com.pokerai.ai.pushfold.PushFoldStrategy
...
class PreFlopStrategy(private val random: Random = Random) {
    ...
    private val pushFoldStrategy = PushFoldStrategy(random)
    ...
    fun decide(player: Player, state: GameState, tournamentState: TournamentState? = null, config: GameConfig? = null, opponentModeler: OpponentModeler? = null): AiDecision {
        ...
        pushFoldStrategy.decide(player, state, gameContext, tournamentState, archetype, profile, opponentModeler, cutoff, scenario)?.let { return it }
